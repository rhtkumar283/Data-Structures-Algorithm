...
public class CSP {
    public static void main(String[] args) {
        ...
        int[] ans  = closestSumPairBruteForce(d1, d2, -15);
    ...
    public static int[] closestSumPairBruteForce(int[] a1, int[] a2, int target){
      int difference = Integer.MAX_VALUE;
      int[] pairs = {a1[0], a2[0]};
      for(int i = 0; i < a1.length; i++){
        for(int j = 0; j < a2.length; j++){
          if(a1[i] + a2[j] == target) return new int[] {a1[i], a2[j]};
          else if(Math.abs(a1[i] + a2[j] - target) < difference){
            difference = Math.abs(a1[i] + a2[j] - target);
            pairs[0] = a1[i];
            pairs[1] = a2[j];
          }
        }
